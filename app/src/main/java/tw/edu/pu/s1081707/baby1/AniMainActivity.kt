...
//    }
    ...
    fun StartPlay(v:View){
        ...
        when (v.id) {
            R.id.ani1_btn -> {
                mper = MediaPlayer.create(this, R.raw.animc1)
                mper.start()
            }

            R.id.ani2_btn -> {
                mper = MediaPlayer.create(this, R.raw.animc2)
                mper.start()
            }

            R.id.ani3_btn -> {
                mper = MediaPlayer.create(this, R.raw.animc3)
                mper.start()
            }

            R.id.ani4_btn -> {
                mper = MediaPlayer.create(this, R.raw.animc4)
                mper.start()
            }

            R.id.ani5_btn -> {
                mper = MediaPlayer.create(this, R.raw.animc5)
                mper.start()
            }

            R.id.ani6_btn -> {
                mper = MediaPlayer.create(this, R.raw.animc6)
                mper.start()
            }

            R.id.ani7_btn -> {
                mper = MediaPlayer.create(this, R.raw.animc7)
                mper.start()
            }

            R.id.ani8_btn -> {
                mper = MediaPlayer.create(this, R.raw.animc8)
                mper.start()
            }
        }
    }
