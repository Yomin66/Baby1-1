...
//    }
    ...
    fun StartPlay(v:View){
        ...
        when (v.id) {
            R.id.ani1_btn -> {
                mper = MediaPlayer.create(this, R.raw.ambutlance_mc)
                mper.start()
            }

            R.id.ani2_btn -> {
                mper = MediaPlayer.create(this, R.raw.carmc2)
                mper.start()
            }

            R.id.sha3_btn -> {
                mper = MediaPlayer.create(this, R.raw.carmc3)
                mper.start()
            }

            R.id.sha4_btn -> {
                mper = MediaPlayer.create(this, R.raw.carmc5)
                mper.start()
            }

            R.id.sha5_btn -> {
                mper = MediaPlayer.create(this, R.raw.carmc7)
                mper.start()
            }

            R.id.sha6_btn -> {
                mper = MediaPlayer.create(this, R.raw.carmc8)
                mper.start()
            }

            R.id.sha7_btn -> {
                mper = MediaPlayer.create(this, R.raw.carmc6)
                mper.start()
            }

            R.id.sha8_btn -> {
                mper = MediaPlayer.create(this, R.raw.carmc4)
                mper.start()
            }
        }
    }
