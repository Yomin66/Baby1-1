...
//    }
    ...
    fun StartPlay(v:View){
        ...
        when (v.id) {
            R.id.ani1_btn -> {
                mper = MediaPlayer.create(this, R.raw.sha1)
                mper.start()
            }

            R.id.ani2_btn -> {
                mper = MediaPlayer.create(this, R.raw.sha2)
                mper.start()
            }

            R.id.sha3_btn -> {
                mper = MediaPlayer.create(this, R.raw.sha3)
                mper.start()
            }

            R.id.sha4_btn -> {
                mper = MediaPlayer.create(this, R.raw.sha5)
                mper.start()
            }

            R.id.sha5_btn -> {
                mper = MediaPlayer.create(this, R.raw.sha7)
                mper.start()
            }

            R.id.sha6_btn -> {
                mper = MediaPlayer.create(this, R.raw.sha8)
                mper.start()
            }

            R.id.sha7_btn -> {
                mper = MediaPlayer.create(this, R.raw.sha6)
                mper.start()
            }

            R.id.sha8_btn -> {
                mper = MediaPlayer.create(this, R.raw.sha4)
                mper.start()
            }
        }
    }
